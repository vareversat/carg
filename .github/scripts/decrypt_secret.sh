...
gpg --batch --yes --decrypt --passphrase="$PASSPHRASE" --output "$GITHUB_WORKSPACE"/android/app/google-services.json "$GITHUB_WORKSPACE"/encrypted_config/google-services.json.gpg
# Decrypt GoogleService-Info.plist.gpg
gpg --batch --yes --decrypt --passphrase="$PASSPHRASE" --output "$GITHUB_WORKSPACE"/ios/Runner/GoogleService-Info.plist "$GITHUB_WORKSPACE"/encrypted_config/GoogleService-Info.plist.gpg
